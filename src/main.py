...
class Handler:
    ...
    @staticmethod
    def button_toggled(cur_btn, con_btn):
    ...
    while True:
        try:
            ...
            while True:
                ...
                GLib.idle_add(update_labels,label_value,str(sc_scale.basic_value),label_time,str(sc_scale.last_seen.strftime('%d.%m.%Y\n%H:%M:%S')))
...
#    builder.connect_signals(Handler())
    ...
    for i in range(1,5):
        ...
        btn1.connect('toggled', Handler.button_toggled, btn2)
        btn2.connect('toggled', Handler.button_toggled, btn1)
    ...
    try:
        ...
        for connection in sc_dbms.ScConnections.select():
            builder.get_object(f'label_r{i}_name').set_text(connection.scale.name)

            thr = threading.Thread(target=update_indicator,args=(
            builder.get_object(f'label_r{i}_value'),builder.get_object(f'label_r{i}_time'), connection.scale.id), daemon=True)
